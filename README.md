...
- **内容增删之后自己填回一页。** 三级间距和字号由两个比例变量驱动，页面加载时二分求解，底部固定留 4px 余量。
- **页面上点着改，改动写回文件。** 存的是纯 HTML，可以 git diff、可以回滚，每次写入前自动备份到 `_backups/`。
- **人和 agent 同时改不会互相覆盖。** 服务端做 mtime 版本校验，版本旧的一方被拒绝，不会静默盖掉。
