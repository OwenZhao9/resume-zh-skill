---
name: resume-zh
description: 把中文简历排成干净克制的一页。字号、三级间距、冒号对齐、链接右对齐都有定死的值，内容增删后版面自动收放保持一页；简历做成浏览器里点着改、改完自动写回文件的 HTML，同时产出 LaTeX 版和 PDF。另含一套「去 AI 味」的文案规则和防止改动互相覆盖的工作流。当用户说「改简历」「简历排版」「简历有 AI 味」「简历转成可编辑的」或要求检查简历格式时使用。
---
